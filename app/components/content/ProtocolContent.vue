<template>
  <div>
    ...
    <div v-if="pending" class="flex items-center justify-center py-12">
      <UIcon name="i-heroicons-arrow-path" class="w-6 h-6 animate-spin text-indigo-600" />
      <span class="ml-2 text-gray-600 dark:text-gray-300">{{ $t('docs.loading.protocol') }}</span>
    </div>
...
// Load protocol content using robust strategy from frameworks/[slug].vue
const { data, error, pending, refresh } = await useAsyncData(
  `protocol-content-${$i18n.locale.value}`,
  async () => {
    try {
      const locale = $i18n.locale.value
      const protocolPath = 'protocol'
      ...
      let content = await queryCollection(locale).path(protocolPath).first()
      
      // If not found, try with leading slash
      if (!content) {
        console.log('🔄 Trying with leading slash...')
        content = await queryCollection(locale).path(`/${protocolPath}`).first()
      }
      
      // If still not found, try with full path including locale
      if (!content) {
        console.log('🔄 Trying with full path...')
        content = await queryCollection(locale).path(`/${locale}/protocol`).first()
      }
      ...
      console.warn(`No content found for protocol in locale ${$i18n.locale.value}`)
      return null
    }
  }
)
