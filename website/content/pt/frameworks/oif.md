...
description: Arquétipos de inteligência artificial para operações e tomada de decisão
navigation:
  title: "OIF"
framework:
  key: oif
...
head:
  meta:
    - name: description
      content: Arquétipos de inteligência artificial para operações e tomada de decisão
    - property: 'og:title'
...
```mermaid
flowchart LR
    subgraph "🤖 Agentes OIF"
        KAgent[Knowledge Agent]
        WAgent[Workflow Agent]
    end
    
    subgraph "🔐 Serviço MOC"
        MOCService{{"Validação de<br/>Autoridade MOC"}}
    end
    
    User[👤 Usuário] --> KAgent
    User --> WAgent
    
    KAgent -->|consulta autoridade| MOCService
    WAgent -->|consulta autoridade| MOCService
    
    MOCService -->|autorizado| Execute[✅ Executar Operação]
    MOCService -->|não autorizado| Escalate[📤 Escalar/Negar]
    
    Execute --> Result[📋 Retornar Resultado<br/>com Explicação MOC]
    Escalate --> Result
```
