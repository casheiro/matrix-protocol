...
description: >-
  Framework conceitual para orchestração de workflows em equipes orientadas por
  IA
navigation:
  title: "ZOF"
framework:
...
head:
  meta:
    - name: description
      content: >-
        Framework conceitual para orchestração de workflows em equipes
        orientadas por IA
    - property: 'og:title'
...
### Estados Canônicos ZOF - Fluxo Completo
...
stateDiagram-v2
    ...
    state "📥 Intake" as Intake {
        ...
        --
        CaptureContext : Captura contexto e requisitos
        OrganizeRequirements : Organiza informações de entrada
    }
    ...
    state "🧠 Understand" as Understand {
        ...
        ProcessUKIs --> [*]
        --
        ConsultOracle : Consulta obrigatória ao Oráculo
        FilterByMOC : Filtra por autoridade/escopo
        ProcessUKIs : Processa UKIs retornadas
    }
    ...
    state "⚖️ Decide" as Decide {
        ...
        MakeDecision --> [*]
        --
        AnalyzeKnowledge : Analisa conhecimento existente
        MakeDecision : Toma decisão baseada em UKIs
    }
    ...
    state "⚡ Act" as Act {
        ...
        CaptureResults --> [*]
        --
        ExecutePlan : Executa ação planejada
        CaptureResults : Captura resultados da execução
    }
    ...
    state "🔍 EvaluateForEnrich" as EvaluateForEnrich {
        ...
        DecisionPoint --> [*] : Aprovado
        DecisionPoint --> InvokeMAL : Conflito detectado
        InvokeMAL --> ApplyMALDecision
        ApplyMALDecision --> [*]
        --
        ApplyMOCCriteria : Aplica critérios organizacionais
        CheckNovelty : Verifica novidade semântica
        ValidateAuthority : Valida autoridade via MOC
        ConflictDetection : Detecta conflitos H1/H2/H3
        DecisionPoint : Decide enriquecimento
        InvokeMAL : Invoca Matrix Arbiter Layer
        ApplyMALDecision : Aplica decisão MAL
    }
    ...
    state "👀 Review" as Review {
        ...
        QualityCheck --> [*]
        --
        ValidateResult : Valida resultado obtido
    ...
    state "📚 Enrich" as Enrich {
        ...
        UpdateOracle --> [*]
        --
        CreateUKI : Cria nova UKI
        ApplyScopeMode : Aplica validação scope_mode
        UpdateOracle : Atualiza base de conhecimento
    }
    ...
    %% Fluxos condicionais pós-avaliação
    ...
    note right of EvaluateForEnrich
        Checkpoint OBRIGATÓRIO
        • Consulta critérios MOC
        • Detecção de conflitos H1/H2/H3
        • Invocação MAL se necessário
        • Validação scope_mode
...
### 🌐 Enriquecimento Multi-scope com Validação de Domínio (Normativo)
