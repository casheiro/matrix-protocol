<template>
  <div>
    ...
    <div v-if="pending" class="flex items-center justify-center py-12">
      ...
      <span class="ml-2 text-gray-600 dark:text-gray-300">{{ $t('docs.loading.integration') }}</span>
    </div>
...
// Load integration content using robust strategy from other content components
const { data, error, pending, refresh } = await useAsyncData(
  `integration-content-${$i18n.locale.value}`,
  async () => {
    try {
      const locale = $i18n.locale.value
      const integrationPath = 'integration'
      
      console.log('🔍 Querying collection:', locale, 'path:', integrationPath)
      ...
      let content = await queryCollection(locale).path(integrationPath).first()
      
      // If not found, try with leading slash
      if (!content) {
        console.log('🔄 Trying with leading slash...')
        content = await queryCollection(locale).path(`/${integrationPath}`).first()
      }
      
      // If still not found, try with full path including locale
      if (!content) {
        console.log('🔄 Trying with full path...')
        content = await queryCollection(locale).path(`/${locale}/integration`).first()
      }
      
      // If still not found, try looking for index file
      if (!content) {
        console.log('🔄 Trying with index...')
        content = await queryCollection(locale).path(`${integrationPath}/index`).first()
      }
      ...
      console.warn(`No content found for integration in locale ${$i18n.locale.value}`)
      return null
    }
  }
)
