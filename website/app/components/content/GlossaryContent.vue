<template>
  <div>
    ...
    <div v-if="pending" class="flex items-center justify-center py-12">
      ...
      <span class="ml-2 text-gray-600 dark:text-gray-300">{{ $t('docs.loading.glossary') }}</span>
    </div>
...
// Load glossary content using robust strategy from frameworks/[slug].vue
const { data, error, pending, refresh } = await useAsyncData(
  `glossary-content-${$i18n.locale.value}`,
  async () => {
    try {
      const locale = $i18n.locale.value
      const glossaryPath = 'glossary'
      
      console.log('🔍 Querying collection:', locale, 'path:', glossaryPath)
      ...
      let content = await queryCollection(locale).path(glossaryPath).first()
      
      // If not found, try with leading slash
      if (!content) {
        console.log('🔄 Trying with leading slash...')
        content = await queryCollection(locale).path(`/${glossaryPath}`).first()
      }
      
      // If still not found, try with full path including locale
      if (!content) {
        console.log('🔄 Trying with full path...')
        content = await queryCollection(locale).path(`/${locale}/glossary`).first()
      }
      ...
      console.warn(`No content found for glossary in locale ${$i18n.locale.value}`)
      return null
    }
  }
)
